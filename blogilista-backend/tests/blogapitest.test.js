...
describe('post test', () => {
    ...
    test('will posting succeed', async () => {
        await api.post('/api/blogs').send(newTestPost)
    ...
    test('no title and url', async () => {
      await api.post('/api/blogs').send(partialPost)
      .expect(400)
    })
})
...
describe('like handling', () => {
  ...
  test('new post without a like property', async () => {
    await api.post('/api/blogs').send(testPostWithoutLike)

    const response = await api.get('/api/blogs')
    const responseBlog = response.body[response.body.length - 1]
...
test('remove a post with id', async () => {
  ...
  await api.delete(blogRefercence)
  const response = await api.get('/api/blogs')
